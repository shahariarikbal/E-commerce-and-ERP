    ...
    <div class="container">
        <div class="row one">
            ...
            <table width="100%">
                <tr class="three">
                    <td align="left" style="width: 50%;vertical-align: top;">
                        <h5>INVOICE NO: #{{ $invoice->purchase->invoice_no }}</h5>
                    </td>
                    <td align="right" style="width: 50%;vertical-align: top;">
                        <h5>DATE: {{ date('m-d-Y', strtotime($invoice->purchase->created_at)) }}</h5>
                    </td>
                </tr>
            </table>
            ...
            <table width="100%">
                <tr class="three">
                    ...
                    <td align="left" style="width: 40%;vertical-align: top;">
                        ...
                        <p>CONTACT NUMBER: {{ $invoice->supplier->mobile_no_one }}</p>
                        <p>ADDRESS: {{ $invoice->supplier->address_one }}</p>
                    </td>
                </tr>
            </table>
            ...
            <table class="table table-bordered" width="100%">
                ...
                <tbody>
                <tr>
                    <th scope="row" style="text-transform: capitalize">{{ isset($invoice) ? $invoice->products->name : '' }}</th>
                    <td style="width: 10%;"><img src="{{ asset('/product/'.$invoice->products->image) }}" style="height: 50px; width: 50px;"></td>
                    <td>{{ isset($invoice->products) ? $invoice->products->sku : '' }}</td>
                    <td>{{ isset($invoice->products) ? $invoice->products->qty : '' }}</td>
                    <td>${{ isset($invoice->products) ? $invoice->products->sale_price : '' }}</td>
                    <td>${{ $grandTotal = $invoice->products->qty * $invoice->products->sale_price }}</td>
                </tr>
                </tbody>
            </table>
                ...
                <tr class="three">
                    <td align="left" style="width: 30%;vertical-align: top;">
                        <h5 style="margin: 8px 0px;">
                            @if(isset($invoice->purchase))
                                {{ $invoice->purchase->created_by }}
                            @else
                            Diplomacy Key
                            @endif
                        </h5>
                        ...
                        <h5 style="margin: 8px 0px;">DATE: {{ date('m-d-Y', strtotime($invoice->purchase->created_at)) }}</h5>
                    </td>
                    ...
                    <td align="left" style="width: 30%; margin: 8px 0px;">
                        ...
                        <h5 style="margin: 8px 0px;">{{ $invoice->supplier->name }}</h5>
                        <h5 style="
									    border-top: 2px solid #676666;
									    padding-top: 8px;margin: 0;
