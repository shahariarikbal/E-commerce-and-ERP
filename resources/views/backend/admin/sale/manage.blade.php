                        ...
                        <div class="col-lg-12">
                            <div class="card">
                                <div class="card-header">
                                    <span style="font-size: 1.25rem; color: black">Sales List</span>
                                    <a href="{{ url('/sale/create') }}" class="btn btn-success float-right" style="margin-right: 15px;"> Add Sale</a>
                                </div>
                                                    ...
                                                    <td>
                                                        <div class="dropdown">
                                                            ...
                                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                                <a href="{{ url('/sale/edit/'.$item->id) }}" title="Sale Edit" class="dropdown-item btn btn-sm btn-success">
                                                                    Edit
                                                                </a>
                                                                <a href="{{ url('/sale/invoice/'.$item->id) }}" title="Sale Invoice" class="dropdown-item btn btn-sm btn-info">
                                                                    Invoice
                                                                </a>
                                                                <a href="{{ url('/sale/delete/'.$item->id) }}" title="Delete Sale" onclick="event.preventDefault();document.getElementById('destroy-form-{{ $item->id }}').submit();" class="dropdown-item btn btn-sm btn-danger">
                                                                    Delete
                                                                </a>
                                                                <form id="destroy-form-{{ $item->id }}" action="{{ url('/sale/delete/'.$item->id) }}" method="post">
                                                                    @csrf
                                                                    @method('delete')
                                                                </form>
                                                            </div>
