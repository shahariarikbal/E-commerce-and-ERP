                ...
                <div class="block-header__body">
                    <nav class="breadcrumb block-header__breadcrumb" aria-label="breadcrumb">
                        <ol class="breadcrumb__list">
                            ...
                            <li class="breadcrumb__item breadcrumb__item--parent"><a href="#" class="breadcrumb__item-link">Manufacture</a></li>
                                                                    ...
                                                                    @foreach($data['categories'] as $category)
                                                                        ...
                                                                        <span id="catId" class="input-check filter-list__input" onclick="myFunction()" >
                                                                            <span class="input-check__body">
                                                                                <input class="input-check__input sub_category" type="checkbox" name="cat" value="{{ $category->first()->category_id }}">
                                                                            ...
                                                                            <span class="filter-list__title" for="{{ $category->first()->category->id }}">{{ $category->first()->category->name }} </span>
                                                                ...
                                                                <div class="filter-list__list"><label class="filter-list__item"></label>
                                                                    @foreach($data['manufactures'] as $manufacture)
                                                                        ...
                                                                        <span class="input-check filter-list__input">
                                                                            <span class="input-check__body">
                                                                                <input class="input-check__input" type="checkbox"  name="man" value="{{ $manufacture->manufacture->id }}">
                                                                            ...
                                                                            <span class="filter-list__title">{{ $manufacture->manufacture->name }} </span>
                                                                        </label>
                                                                        @php
                                                                            break;
                                                                        @endphp
...
{{--                                 <div class="block block-brands block-brands--layout--columns-5-full">
    ...
    <script>
        ...
        function myFunction() {   
            ...
            $.ajax({
                url: "{{ url('/manufacture/product/filtering/') }}/"+ id,
        ...
        function myFunction2() {
            ...
            $.ajax({
                url: "{{ url('/manufacture/product/filtering/') }}/"+ brandId + slash +min.innerHTML + slash + max.innerHTML + '?' + 'category=' + category + optional + 'sub_category=' +subCategory + optional + 'manufacture=' + manufacture + optional + 'condition=' + condition + optional + 'company=' + company,
                ...
                success: function (data) {
                    ...
                    for (var i = 0; i < data.products.length; i++) {
                        ...
                        html+='<div class="product-card">'
                        html+='<div class="product-card__actions-list">'
                        html+='<button class="product-card__action product-card__action--wishlist addWiseList" onclick="addToWishList('+data.products[i].id+')" type="button" aria-label="Add to wish list"><svg width="16" height="16"><path d="M13.9,8.4l-5.4,5.4c-0.3,0.3-0.7,0.3-1,0L2.1,8.4c-1.5-1.5-1.5-3.8,0-5.3C2.8,2.4,3.8,2,4.8,2s1.9,0.4,2.6,1.1L8,3.7l0.6-0.6C9.3,2.4,10.3,2,11.3,2c1,0,1.9,0.4,2.6,1.1C15.4,4.6,15.4,6.9,13.9,8.4z"/></svg></button>'
                        html+='</div>'

                        html+='<div class="product-card__image">'
                        html+='<a href="'+details+data.products[i].id+'">'
                        html+='<img src="'+baseUrl+'product/'+data.products[i].image+'">'
                        html+='</a>'
                        html+='</div>'

                        html+='<div class="product-card__info">'
                        html+='<div class="product-card__meta"><span class="product-card__meta-title">SKU:</span>'
                        html+=data.products[i].sku
                        html+='</div>'
                        ...
                        if (data.products[i].sale != null) {
                            html+='<div class="tag-badge tag-badge--sale">'
                            html+=data.products[i].sale
                            html+='</div>'
                        }

                        if (data.products[i].hot != null) {
                            html += '<div class="tag-badge tag-badge--hot">'
                            html += data.products[i].hot
                            html += '</div>'
                        }

                        if (data.products[i].new != null) {
                            html += '<div class="tag-badge tag-badge--new">'
                            html += data.products[i].new
                            html += '</div>'
                        }
                        ...
                        html+='<div class="product-card__prices">'
                        html+='<div class="product-card__price product-card__price--current">$'
                        html+=data.products[i].sale_price
                        html+='</div>'
                        html+='<div class="product-card__price product-card__price--old">$'
                        html+=data.products[i].pre_price
                        html+='</div>'
                        html+='</div>'
                        html+='<button class="product-card__addtocart-icon" onclick="addToCart('+data.products[i].id+')" type="button" aria-label="Add to cart"><svg width="20" height="20"><circle cx="7" cy="17" r="2"/><circle cx="15" cy="17" r="2"/><path d="M20,4.4V5l-1.8,6.3c-0.1,0.4-0.5,0.7-1,0.7H6.7c-0.4,0-0.8-0.3-1-0.7L3.3,3.9C3.1,3.3,2.6,3,2.1,3H0.4C0.2,3,0,2.8,0,2.6V1.4C0,1.2,0.2,1,0.4,1h2.5c1,0,1.8,0.6,2.1,1.6L5.1,3l2.3,6.8c0,0.1,0.2,0.2,0.3,0.2h8.6c0.1,0,0.3-0.1,0.3-0.2l1.3-4.4C17.9,5.2,17.7,5,17.5,5H9.4C9.2,5,9,4.8,9,4.6V3.4C9,3.2,9.2,3,9.4,3h9.2C19.4,3,20,3.6,20,4.4z"/></svg></button>'
                        html+='<button class="product-card__addtocart-full" type="button">Add to cart</button>'
