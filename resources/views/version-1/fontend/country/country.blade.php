                ...
                <div class="block-header__body">
                    <nav class="breadcrumb block-header__breadcrumb" aria-label="breadcrumb">
                        <ol class="breadcrumb__list">
                            ...
                            <li class="breadcrumb__item breadcrumb__item--parent"><a href="#" class="breadcrumb__item-link">Country</a></li>

                            <li class="breadcrumb__item breadcrumb__item--current breadcrumb__item--last" aria-current="page"><span class="breadcrumb__item-link">{{ isset($countryName->name) ? $countryName->name : '' }}</span></li>

                            <li class="breadcrumb__title-safe-area" role="presentation"></li>
                        </ol>
                    </nav>

                    <h1 class="block-header__title">{{ $countryName->name }}</h1>
                </div>
            </div>
        </div>
                            ...
                            <div class="products-view">
                                ...
                                <div class="products-view__list products-list products-list--grid--3" data-layout="grid" data-with-features="false">
                                    ...
                                    <div class="products-list__content" id="initialProducts">
                                        @if(!empty($categoryWishProducts))
                                            @foreach($categoryWishProducts as $countryProduct)
                                                @if(!empty($countryProduct->products))
                                                    <div class="products-list__item">
                                                        <div class="product-card">
                                                            <div class="product-card__actions-list">
                                                                <button class="product-card__action product-card__action--wishlist addWiseList" data-id="{{ isset($countryProduct->products->name->id) ? $countryProduct->products->name->id : '' }}" type="button" aria-label="Add to wish list"><svg width="16" height="16"><path d="M13.9,8.4l-5.4,5.4c-0.3,0.3-0.7,0.3-1,0L2.1,8.4c-1.5-1.5-1.5-3.8,0-5.3C2.8,2.4,3.8,2,4.8,2s1.9,0.4,2.6,1.1L8,3.7l0.6-0.6C9.3,2.4,10.3,2,11.3,2c1,0,1.9,0.4,2.6,1.1C15.4,4.6,15.4,6.9,13.9,8.4z"/></svg>
                                                                </button>
                                                            </div>

                                                            <div class="product-card__image">
                                                                <a href="{{ url('/upcoming/product/details/'.$countryProduct->products->id) }}">
                                                                    <img src="{{ asset('/public/product/'.$countryProduct->products->image) }}" alt="">
                                                                </a>
                                                            </div>

                                                            <div class="product-card__info">
                                                                <div class="product-card__meta"><span class="product-card__meta-title">SKU:</span> {{ $countryProduct->products->sku }}</div>

                                                                <div class="product-card__name">
                                                                    <div>
                                                                        <div class="product-card__badges">
                                                                            @if($countryProduct->products->sale)
                                                                                <div class="tag-badge tag-badge--sale">{{ $countryProduct->products->sale }}</div>
                                                                            @endif
                                                                            @if($countryProduct->products->hot)
                                                                                <div class="tag-badge tag-badge--hot">{{ $countryProduct->products->hot }}</div>
                                                                            @endif
                                                                            @if($countryProduct->products->new)
                                                                                <div class="tag-badge tag-badge--new">{{ $countryProduct->products->new }}</div>
                                                                            @endif
                                                                        </div>
                                                                        <a href="#">{{ $countryProduct->products->name }}</a>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            ...
                                                            <div class="product-card__footer">
                                                                <div class="product-card__prices">
                                                                    <div class="product-card__price product-card__price--current">${{ number_format($countryProduct->products->sale_price,2) }}</div>
                                                                    <div class="product-card__price product-card__price--old">${{ number_format($countryProduct->products->pre_price,2) }}</div>
                                                                </div>
                                                                <button class="product-card__addtocart-icon addCart" type="button" data-id="{{ $countryProduct->products->id }}" aria-label="Add to cart"><svg width="20" height="20"><circle cx="7" cy="17" r="2"/><circle cx="15" cy="17" r="2"/><path d="M20,4.4V5l-1.8,6.3c-0.1,0.4-0.5,0.7-1,0.7H6.7c-0.4,0-0.8-0.3-1-0.7L3.3,3.9C3.1,3.3,2.6,3,2.1,3H0.4C0.2,3,0,2.8,0,2.6V1.4C0,1.2,0.2,1,0.4,1h2.5c1,0,1.8,0.6,2.1,1.6L5.1,3l2.3,6.8c0,0.1,0.2,0.2,0.3,0.2h8.6c0.1,0,0.3-0.1,0.3-0.2l1.3-4.4C17.9,5.2,17.7,5,17.5,5H9.4C9.2,5,9,4.8,9,4.6V3.4C9,3.2,9.2,3,9.4,3h9.2C19.4,3,20,3.6,20,4.4z"/></svg>
                                                                </button>

                                                                <button class="product-card__addtocart-full" type="button">Add to cart</button>

                                                                <button class="product-card__wishlist addWiseList" data-id="{{ $countryProduct->products->id }}" type="button"><svg width="16" height="16"><path d="M13.9,8.4l-5.4,5.4c-0.3,0.3-0.7,0.3-1,0L2.1,8.4c-1.5-1.5-1.5-3.8,0-5.3C2.8,2.4,3.8,2,4.8,2s1.9,0.4,2.6,1.1L8,3.7l0.6-0.6C9.3,2.4,10.3,2,11.3,2c1,0,1.9,0.4,2.6,1.1C15.4,4.6,15.4,6.9,13.9,8.4z"/></svg> <span>Add to wishlist</span></button>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    @endif
