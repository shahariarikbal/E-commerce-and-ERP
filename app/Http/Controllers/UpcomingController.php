...
class UpcomingController extends Controller
{
    public function upcomingProducts()
    {
        ...
        return view('version-1.fontend.upcoming.upcoming', compact('countries','conditions', 'webLatestProducts','subcategoriesCount','maxPrice','minPrice','cartProducts','webCategories','webBrands','webManufacturers','webProducts','webCategory','webUpcomingProducts'));
    }
    ...
    public function discountProducts()
    {
        $webCategory = Category::with('product')->withCount('product')->get();
        $webCategories = WebCategory::with('category')->get();
        $webUpcomingProducts = Product::where('status', 1)->where('product_type', 'Discount')->orderBy('id', 'DESC')->get();
        $webBrands = WebBrand::with('product')->withCount('product')->get();
        $webManufacturers = WebManufacture::with('product')->withCount('product')->get();
        ...
        $conditions = Condition::with('products')->withCount('products')->get();
        $countries = Country::all();
        return view('version-1.fontend.discount.discount', compact('countries','conditions','webLatestProducts','subcategoriesCount','maxPrice','minPrice','cartProducts','webCategories','webBrands','webManufacturers','webProducts','webCategory','webUpcomingProducts'));
    }
}
