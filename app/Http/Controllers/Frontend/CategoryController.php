...
class CategoryController extends Controller
{
    public function categoryWishProducts($id)
    {
        ...
        $categoryProduct = ProductCategory::where('category_id', $id)->get();

        $productBrand = ProductBrand::with('brand', 'product')->whereIn('product_id', $categoryProduct->pluck('product_id'))->get()->groupBy('brand_id');
        $productSubCategory = ProductSubcategory::with('subcategory', 'product')->whereIn('product_id', $categoryProduct->pluck('product_id'))->get()->groupBy('subcategory_id');
        $productManufacture = ProductManufacture::with('manufacture', 'product')->whereIn('product_id', $categoryProduct->pluck('product_id'))->get()->groupBy('manufacture_id');
        ...
        return view('version-1.fontend.category.products', compact(
    ...
    public function sliderPrice($categoryId, Request $request)
    {
        // return $categoryId;
        ...
        $max = $request->maximum;
        $products = array();
        $productCategories = ProductCategory::with('product')->where('category_id', $categoryId)->get();
        ...
        // return $productCategories[1]->product->sale_price;
        foreach ($productCategories as $pb) {
    ...
    public function sliderPrice2($categoryId, $min, $max, Request $request)
    {
        $productCat = [];
        $productSubCat = [];
        $productManufacture = [];
        ...
        $productId = ProductCategory::where('category_id', $categoryId)->pluck('product_id');
        $query = Product::whereIn('id', $productId);
        $query = $query->where('sale_price', '>=', $min)->where('sale_price', '<=', $max);

        $categoryAllProducts = $query->get()->toArray();

        $products = array_merge($categoryAllProducts, $categoryProducts, $subCategoryProducts, $manufactureProducts, $brandWishProducts);
